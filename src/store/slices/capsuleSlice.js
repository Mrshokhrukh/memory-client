  ...
  async ({ page = 1, limit = 10 } = {}, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || 'Failed to fetch capsules'
      );
    }
  }
);
...
export const fetchCapsuleById = createAsyncThunk(
  ...
  async (capsuleId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || 'Failed to fetch capsule'
      );
    }
  }
);
...
export const createCapsule = createAsyncThunk(
  ...
  async (capsuleData, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || 'Failed to create capsule'
      );
    }
  }
);
...
export const joinCapsule = createAsyncThunk(
  ...
  async ({ capsuleId, inviteCode }, { rejectWithValue }) => {
    try {
      const response = await api.post(`/capsules/${capsuleId}/join`, {
        inviteCode,
      });
      return response.data.data.capsule;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || 'Failed to join capsule'
      );
    }
  }
);

export const leaveCapsule = createAsyncThunk(
  'capsules/leaveCapsule',
  async (capsuleId, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || 'Failed to leave capsule'
      );
    }
  }
);
  ...
  async ({ page = 1, limit = 12, search = '' } = {}, { rejectWithValue }) => {
    try {
      const response = await api.get(
        `/capsules/explore/public?page=${page}&limit=${limit}&search=${search}`
      );
      return response.data.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || 'Failed to fetch public capsules'
      );
    }
  }
);
...
const capsuleSlice = createSlice({
  ...
  reducers: {
    ...
    removeUserFromCapsule: (state, action) => {
      ...
      if (state.currentCapsule && state.currentCapsule._id === capsuleId) {
        state.currentCapsule.contributors =
          state.currentCapsule.contributors.filter(
            (c) => c.user._id !== userId
          );
        state.currentCapsule.stats.totalContributors -= 1;
      }
    ...
    builder
      ...
      .addCase(updateCapsule.fulfilled, (state, action) => {
        ...
        if (
          ...
          state.currentCapsule._id === updatedCapsule._id
        ) {
      ...
      .addCase(joinCapsule.fulfilled, (state, action) => {
        ...
        const existingIndex = state.capsules.findIndex(
          (c) => c._id === joinedCapsule._id
        );
        if (existingIndex === -1) {
